...
struct VoiceShortcut: Equatable {
    ...
    static func parse(_ value: String) -> VoiceShortcut? {
        let parts = value
            .lowercased()
            .split(separator: "+")
            .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
            .filter { !$0.isEmpty }
        guard parts.count >= 2, let keyName = parts.last, let keyCode = keyCodes[keyName] else {
            return nil
        }
        ...
        return VoiceShortcut(keyCode: keyCode, flags: flags)
    }
