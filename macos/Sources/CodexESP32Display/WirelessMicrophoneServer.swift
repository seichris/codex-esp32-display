...
final class WirelessPairingStore: @unchecked Sendable {
    ...
    func importBundle(_ data: Data) throws -> WirelessPairing {
        ...
        if let encodedIdentity = bundle.serverIdentityPKCS12Base64,
           ...
           let identity = Data(base64Encoded: encodedIdentity) {
            try importServerIdentity(identity, password: passphrase, certificatePEM: bundle.serverCertificatePEM)
    ...
    func importServerIdentity(_ data: Data, password: String, certificatePEM: String) throws {
        guard let expected = Self.certificateDER(certificatePEM) else {
            throw WirelessMicrophoneServerError.invalidPairing
        }
        var items: CFArray?
        ...
        guard status == errSecSuccess,
              ...
              let value = array.first?[kSecImportItemIdentity as String],
              CFGetTypeID(value as CFTypeRef) == SecIdentityGetTypeID() else {
            throw WirelessMicrophoneServerError.invalidPairing
        }
        let identity = value as! SecIdentity
        guard Self.matches(identity, certificate: expected),
              findIdentity(certificate: expected) != nil else {
            throw WirelessMicrophoneServerError.invalidPairing
        }
    }

    static func certificateDER(_ pem: String) -> Data? {
        ...
        let body = text.dropFirst(prefix.count).dropLast(suffix.count)
    ...
    private static func matches(_ identity: SecIdentity, certificate: Data) -> Bool {
        var actual: SecCertificate?
        guard SecIdentityCopyCertificate(identity, &actual) == errSecSuccess,
              let actual else { return false }
        return SecCertificateCopyData(actual) as Data == certificate
    }

    private func findIdentity(certificate: Data) -> SecIdentity? {
        // Labels are not unique. Pairing rotation can leave multiple identities
        // with the same label; the exact provisioned certificate is authoritative.
        ...
        return identities.first { Self.matches($0, certificate: certificate) }
    }

    func serverIdentity() -> SecIdentity? {
        guard let pem = defaults.string(forKey: "WirelessMicrophone.certificatePEM"),
              let certificate = Self.certificateDER(pem) else { return nil }
        return findIdentity(certificate: certificate)
    }
