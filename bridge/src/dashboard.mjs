...
const tokenStorageKey='codex-esp32-display-token';
const tokenFromUrl=new URLSearchParams(window.location.hash.slice(1)).get('token')||'';
if(tokenFromUrl){localStorage.setItem(tokenStorageKey,tokenFromUrl);history.replaceState(null,document.title,window.location.pathname+window.location.search)}
...
const auth=()=>{const token=localStorage.getItem(tokenStorageKey)||'';return token?{Authorization:'Bearer '+token}:{}};
...
async function authorizedFetch(path){let response=await fetch(path,{cache:'no-store',headers:auth()});if(response.status===401){const token=prompt('Bridge token')||'';if(token){localStorage.setItem(tokenStorageKey,token);response=await fetch(path,{cache:'no-store',headers:auth()})}}return response}
...
document.querySelector('#token').onclick=()=>{const token=prompt('Bridge token',localStorage.getItem(tokenStorageKey)||'');if(token!==null){if(token)localStorage.setItem(tokenStorageKey,token);else localStorage.removeItem(tokenStorageKey);tick()}};
