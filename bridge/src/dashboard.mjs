...
async function openDetail(item){list.hidden=true;detail.hidden=false;detailTitle.textContent=item.title;detailMeta.textContent=item.project;detailKind.textContent='Loading latest text';detailText.textContent='Loading…';detailBadges.innerHTML=item.reasons.map(badge).join('');try{const response=await authorizedFetch('/api/v1/threads/'+encodeURIComponent(item.id)+'/latest');if(!response.ok)throw new Error('HTTP '+response.status);const data=await response.json();detailTitle.textContent=data.title;detailMeta.textContent=data.project;detailKind.textContent=(data.kind||'latest')+(data.truncated?' · truncated':'');detailText.textContent=data.text;detailBadges.innerHTML=(data.reasons||[]).map(badge).join('')}catch(error){detailKind.textContent='Could not load';detailText.textContent=error.message}}
async function load(){const response=await authorizedFetch('/api/v1/attention');if(!response.ok)throw new Error('HTTP '+response.status);const data=await response.json();count.textContent=data.totalCount+' item'+(data.totalCount===1?'':'s');errorBox.innerHTML=data.diagnostics?.sourceError?'<div class="error">'+esc(data.diagnostics.sourceError)+'</div>':'';if(!data.items.length){list.innerHTML='<div class="empty"><strong>Inbox clear.</strong><br>No unread, pinned, or waiting threads.</div>';return}list.innerHTML=data.items.map((item,index)=>'<button type="button" class="card '+esc(item.status)+'" data-index="'+index+'"><div class="title">'+esc(item.title)+'</div><div class="meta"><span>'+esc(item.project)+'</span><span>·</span><span>'+age(item.ageSeconds)+'</span></div>'+(item.preview&&item.preview!==item.title?'<div class="preview">'+esc(item.preview)+'</div>':'')+'<div class="badges">'+item.reasons.map(badge).join('')+(item.status==='running'?'<span class="badge run">Running</span>':'')+'</div></button>').join('');list.querySelectorAll('.card').forEach(button=>button.addEventListener('click',()=>openDetail(data.items[Number(button.dataset.index)])))}
