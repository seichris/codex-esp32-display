...
static void init_pwr_button(void)
{
    ...
    if (result == ESP_OK) result = pmu_write(AXP2101_INTEN2, enabled | AXP2101_PKEY_SHORT_MASK);
    if (result == ESP_OK) result = pmu_write(AXP2101_INTSTS2, AXP2101_PKEY_SHORT_MASK);
    if (result != ESP_OK) {
        ESP_LOGW(TAG, "Could not initialize AXP2101 PKEY status: %s", esp_err_to_name(result));
    } else {
        ESP_LOGI(TAG, "PWR short-press input ready");
    }
}
    ...
    *event = BUTTON_INPUT_NONE;
    ...
    if (s_pmu != NULL && (now - s_last_pwr_poll) >= pdMS_TO_TICKS(PWR_POLL_MS)) {
        ...
        if (pmu_read(AXP2101_INTSTS2, &status) == ESP_OK
            && (status & AXP2101_PKEY_SHORT_MASK) != 0) {
            (void)pmu_write(AXP2101_INTSTS2, AXP2101_PKEY_SHORT_MASK);
            *event = BUTTON_INPUT_SELECT;
            return true;
        }
    }
